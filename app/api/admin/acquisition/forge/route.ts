...
/**
 * THE ACQUISITION FORGE BOARD.
 *
 * The outbound cockpit has had a forge board since July. Acquisition has had
 ...
 * they cost money:
 *   1. Who showed interest and never gave a number? (Forge for them.)
...
export const SEGMENT_LABELS: Record<ForgeSegment, string> = {
...
export const SEGMENT_NOTES: Record<ForgeSegment, string> = {
  ...
  cold: 'In the campaign, nothing forged, no movement yet.',
  closed: 'They bought, opted out, or were lost. Work stops.',
};

/* ────────────────────────────── the row ────────────────────────────────── */

export type AcqForgeRow = {
  id: string;
...
export type AcqForgeCounts = Record<ForgeSegment | 'all', number>;
...
/** Anything forged for the prospect: voice agent, website, command center, or hub. */
...
function segmentFor(lead: AcqProspect, suite: SuiteState, move: Movement): ForgeSegment {
  ...
  // Nothing forged. Rank by how close they already got on their own.
...
export async function GET(req: Request) {
  ...
  let forged: AcqProspect[] = [];
  let movement: Map<string, Movement>;
  try {
    [forged, movement] = await Promise.all([
    ...
    return NextResponse.json({ error: e instanceof Error ? e.message : 'Could not read the forge board.' }, { status: 500 });
  }

  const byId = new Map<string, AcqProspect>(forged.map((l) => [l.id, l]));
  ...
  const siteById = new Map<string, AcqForgeRow['siteRun']>();
  for (const c of chunk(siteIds)) {
    const { data } = await db
      .from('outbound_demo_sites')
      .select('id, status, kind, error, created_at, claimed_at, built_at')
      .in('id', c);
    for (const r of (data ?? []) as NonNullable<AcqForgeRow['siteRun']>[]) siteById.set(r.id, r);
  }

  const videos = await personalVideoIds(db);

  const rows: AcqForgeRow[] = leads.map((l) => {
  ...
  const counts = rows.reduce<AcqForgeCounts>(
    (acc, r) => {
      acc[r.segment] += 1;
      acc.all += 1;
      return acc;
  ...
  const rank: Record<ForgeSegment, number> = {
  ...
  return NextResponse.json({
    rows,
    counts,
    worker: await readForgeWorkerVitals(db),
...
/**
 * How many suites one request will forge on the spot. Each one mints a Vapi
...
export async function POST(req: Request) {
  ...
  switch (action) {
    /* Forge the whole suite. Small batches run now; big ones go on the queue. */
    case 'forge': {
      if (ids.length > INLINE_FORGE_LIMIT) {
        let queued = 0;
        for (const id of ids) {
        ...
        return NextResponse.json({
          ...
          note:
            queued === 0
              ...
              : `${queued} suite${queued === 1 ? '' : 's'} queued. The engine builds them on its next pass, and the websites land as your forge works through them.`,
        });
      }
      ...
      for (const lead of leads) {
        const res = await forgeProspectSuite(db, lead, {
          site: withSite,
          designTier,
          talkingWebsite,
          forceSite: force,
        ...
        results.push({
          ...
          note: res.ok
            ? res.created.length
              ? `Forged ${res.created.join(', ')}${res.warnings.length ? `. ${res.warnings[0]}` : ''}`
              : 'Already built'
            : res.error,
        });
      }
      ...
      return NextResponse.json({
        ...
        queued: 0,
        results,
        note: `${built} of ${results.length} forged.${withSite ? ' Their websites are on the anvil now.' : ''}`,
      });
    }
      ...
      await Promise.all(
        ids.map((id) =>
          recordEvent(db, {
            ...
            campaignId: campaign?.id ?? null,
            type: 'note',
            label: 'Their suite email was queued from the forge board',
          }),
        ),
      );
