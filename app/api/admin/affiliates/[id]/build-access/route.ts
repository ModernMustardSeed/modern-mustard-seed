/**
 * Owner-only governance for the partner build: grant/revoke can_forge and set
 * the per-partner caps. Granting build access grants spend, so this is the one
...
export async function POST(req: Request, { params }: { params: Params }) {
  ...
  if (body.action === 'grant' || body.action === 'revoke') {
    ...
    return NextResponse.json({ ok: true, canBuild: body.action === 'grant' });
  }
