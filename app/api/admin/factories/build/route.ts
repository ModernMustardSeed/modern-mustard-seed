...
import { researchBusiness, recommendTemplate, buildBlueprint, outstandingQuestions } from '@/lib/factory/build';
...
export async function POST(req: Request) {
  ...
  const parsed = await parseJson(req, buildSchema);
  ...
  if (!built.ok) {
    return NextResponse.json(
      { error: built.error, queuedJobId: built.queuedJobId ?? null, research: summarize(research), recommendation },
      { status: built.queuedJobId ? 202 : 400 },
    );
  }
  ...
  const fit = fitScore({
    ...
    regulated: built.result.ok ? built.result.doc.compliance.regulated_vertical : false,
    integrationCount: built.result.ok ? built.result.doc.integrations.length : 0,
  });
  ...
  await audit(guard.supabase, {
    ...
    target: research.url,
    meta: { template: built.templateKey, fit: fit.score, valid: built.result.ok },
  });
  ...
  return NextResponse.json({
    ...
    blueprint: built.result.ok ? built.result.doc : null,
    issues: built.result.ok ? [] : built.result.errors,
    complexity: built.result.ok ? implementationComplexity(built.result.doc) : null,
    fit,
    fitNotes: built.fitNotes,
    outstanding: outstandingQuestions(input.answers, built.missing),
  });
}
