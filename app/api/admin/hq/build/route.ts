/**
 * Team-facing mint ("Build for a business you know" in the admin Partner Hub).
...
import { mintBuiltSuite, isoWeekKey } from '@/lib/partner-build';
...
export async function GET() {
  ...
  const [usedToday, usedWeek, mintsRes] = await Promise.all([
    usedCount(supabase, `partnerbuild:${minterKey}:day:${day}`),
    usedCount(supabase, `partnerbuild:${minterKey}:week:${isoWeekKey(new Date())}`),
    supabase
      .from('outbound_leads')
...
export async function POST(req: Request) {
  ...
  const result = await mintBuiltSuite(supabase, 'rep', {
    ...minter,
