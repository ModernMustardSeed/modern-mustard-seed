...
import { sendBuildHandoffEmail, PARTNER_BUILD_QA_LIFT } from '@/lib/partner-build';
...
export async function GET() {
  ...
  const pending = ((data || []) as unknown as PendingRow[]).map((r) => ({
    ...
    partner: r.affiliates
      ? { name: r.affiliates.name, email: r.affiliates.email, code: r.affiliates.code, approved: r.affiliates.forge_qa_approved, lift: PARTNER_BUILD_QA_LIFT }
      : null,
  }));
  ...
  if (lead.affiliate_id) {
    ...
    if (aff) {
      ...
      if (process.env.RESEND_API_KEY && lead.hub_demo_url) {
        try {
          await sendBuildHandoffEmail(resendClient(), {
          ...
          console.error('build-qa hand-off email failed', err);
        }
      }
    }
  }
