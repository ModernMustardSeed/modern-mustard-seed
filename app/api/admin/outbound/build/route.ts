...
/**
 * The BUILD BOARD: every lead that has work already invested in it.
 *
 * A built suite is money already spent. The cockpit could show you a demo the
 * moment you built it and then lose it forever in a floor of thousands, which
...
/** Anything built for the lead: voice agent, website, command center, or the suite hub. */
const BUILT_FILTER = 'demo_run_id.not.is.null,site_demo_id.not.is.null,os_demo_id.not.is.null,hub_demo_id.not.is.null';
...
type SiteRow = BuildSiteRun & { lead_id: string; business_name: string };
...
export async function GET(req: Request) {
  ...
  // A part-time caller sees only their own built leads, same scoping as the floor.
  ...
  // Build rows, call history, and command-center build times for exactly these leads.
  ...
  const rows: BuildRow[] = leads.map((l) => {
    const siteRow = siteByLead.get(l.id) ?? null;
    const site: BuildSiteRun | null = siteRow
      ? {
          id: siteRow.id,
    ...
    // The voice agent and the hub carry no build timestamp of their own, so we
    ...
    return {
      ...
      integration_plan_url: l.integration_plan_url,
      integration_plan_status: l.integration_plan_status,
      presence_audit_url: l.presence_audit_url,
      presence_audit_score: l.presence_audit_score,
  ...
  const counts = rows.reduce<BuildCounts>(
    (acc, r) => {
      acc[r.stage] += 1;
      acc.all += 1;
      return acc;
  ...
  if (summaryOnly) return NextResponse.json({ counts, worker: await readBuildWorkerVitals(guard.supabase) });

  // Work order, not chronology: what is building, what broke, what is built and
  // unspent, then the rest. Inside a bucket, the freshest build leads.
  const rank: Record<BuildStage, number> = { forging: 0, failed: 1, uncontacted: 2, waiting: 3, landed: 4, closed: 5 };
  rows.sort((a, b) => {
  ...
  return NextResponse.json({ rows, counts, reps, worker: await readBuildWorkerVitals(guard.supabase) });
}
