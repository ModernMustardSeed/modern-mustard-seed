...
import { buildLeadVoiceDemo, buildOsConfig, captureLeadBrand, ensureDemoHub } from '@/lib/outbound-demo';
...
/**
 * The third build: the BUSINESS OS demo, for the lead who says "I have the
...
export async function POST(_req: Request, { params }: { params: Params }) {
  ...
  const { data: row, error: insErr } = await guard.supabase
    ...
    .single();
  if (insErr || !row) {
    return NextResponse.json({ error: insErr?.message ?? 'Could not build the OS demo.' }, { status: 500 });
  }
  ...
  await guard.supabase.from('messages').insert({
    ...
    subject: 'Business OS demo built',
