...
import { buildLeadVoiceDemo, buildOsConfig } from '@/lib/outbound-demo';
 ...
 *   - site: re-queue their demo WEBSITE as an edit (the build preserves everything and
...
export async function POST(req: Request, { params }: { params: Params }) {
  ...
  if (!['site', 'os', 'voice'].includes(target ?? '')) {
    return NextResponse.json({ error: 'Pick what to rebuild: site, os, or voice.' }, { status: 400 });
  }
  ...
  if (target === 'voice') {
    const res = await buildLeadVoiceDemo(guard.supabase, l, { instruction, force: true });
  ...
  if (target === 'site') {
    ...
    // brief is overwritten by a later rebuild or a fresh build (learned 2026-07-16, when
    // Polly's instructions survived only because the failed edit row still held them).
    await note('Website rebuild queued', `Rebuilding their website from your prompt: "${instruction}"`);
  ...
  if (!l.os_demo_id) {
    return NextResponse.json({ error: 'There is no business OS demo yet. Build one first, then rebuild it.' }, { status: 400 });
  }
  ...
  // build-time brand capture; carry the stored logo + brand color forward or a
  // rebuild would silently strip them.
  ...
  await note('Business OS rebuilt', `Their command center was remapped from your prompt: "${instruction}"`);
 ...
 * The OS is a template, so a rebuild here is a config remap, not a rebuild: only the
...
Only include a field if the change request clearly asks to change it. If nothing should change, reply {}.`;
  ...
  try {
    ...
    return { ok: false, error: `The OS rebuild model call failed: ${(e as Error)?.message ?? e}` };
  }
