...
/**
 * Lists the FINISHED cuts: edited videos Claude drops into the CXC bucket's
 * `finals/` folder, ready to post. This is the "where did my edited video go"
 * surface, shown right in the CXC booth so the loop is visible where Sarah records.
 * CXC has no one-button publisher: these cuts go out to Instagram and X by hand,
 * which is why nothing here links to the MMS YouTube publisher. Same-origin guard only.
 */

const BUCKET = 'booth-cxc';
const FINALS = 'finals';
