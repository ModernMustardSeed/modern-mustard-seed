/**
 * The Voice Agent Build API.
 *
 * mode "web"  : build the persona, record the run, return assistantOverrides
 ...
 *   - 1 build per email, atomic via app_state pk claim (lib/demo-run-store)
...
import { buildCall, ringDemoCall, toE164, type DemoAgentProfile } from '@/lib/demo-agent';
import {
  claimEmail,
  claimPhone,
  claimRing,
...
export async function POST(req: Request) {
  ...
  return handleBuild(supabase, body, ip);
}
...
async function handleBuild(
  ...
  // Cap 1: one build per email, forever. Atomic (pk claim), fails closed.
  ...
  if (emailClaim === 'taken') {
    return NextResponse.json(
      { error: 'already_forged', message: 'You already built your Voice Agent. Ready to put him on your real phones? That is the button below.' },
      { status: 402 }
    );
  }
  ...
  if (todayCount > GLOBAL_DAILY_CAP) {
    await releaseKey(supabase, 'email', email);
    await notifySarah('VOICE AGENT BUILD: daily cap reached', [
      `The build hit its ${GLOBAL_DAILY_CAP}-demo daily cap. Latest attempt: ${business} (${email}).`,
      'Raise GLOBAL_DAILY_CAP in app/api/demo-agent/build/route.ts if this is good traffic.',
    ]);
    return NextResponse.json(
      { error: 'forge_cooling', message: 'The build is cooling down after a busy day. Come back tomorrow, or call Mr. Mustard right now at ' + DEMO_AGENT.phoneLine + '.' },
      { status: 429 }
    );
  }
  ...
  if (!built.ok) {
    ...
    await releaseKey(supabase, 'email', email);
    console.error('demo agent build failed', built.error);
    return NextResponse.json({ error: 'forge_offline' }, { status: 503 });
  }
  ...
  try {
    await insertLead({
      ...
      // column and the Voice Agent build is a no-DDL surface, so the token is
  ...
  await notifySarah(`VOICE AGENT BUILT: ${business} (${city})`, [
    `<strong>${esc(ownerName)}</strong> just built a Voice Agent for <strong>${esc(business)}</strong> (${getVertical(verticalId).label}, ${esc(city)}).`,
    `Email: ${esc(email)}`,
    `Taught him: ${esc(services.slice(0, 300))}`,
    `Run ${runId}. Transcript lands in the Vapi dashboard under metadata kind=demo-agent.`,
  ]);

  return NextResponse.json({ runId, call: built.call, phoneLine: DEMO_AGENT.phoneLine });
}
...
async function handleRing(
  ...
  const built = await buildCall(profile, runId, 'phone');
  if (!built.ok) {
