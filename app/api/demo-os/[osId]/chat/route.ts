...
export async function POST(req: Request, { params }: { params: Params }) {
  ...
  if (!sb) return NextResponse.json({ error: 'Demo assistant is warming up.' }, { status: 503 });
  ...
  const dataSummary = [
    ...
    `Overnight calls the voice agent caught: ${preset.overnightCalls.map((c) => `${c.caller} at ${c.time}: ${c.need} -> ${c.outcome}`).join('; ')}.`,
    ...
    const reply = await llmText({
      ...
      user:
        mode === 'ad'
          ? 'Write the ad now.'
          : mode === 'campaign'
            ? 'Design the campaign now.'
            : renderTranscript(history, { assistantLabel: 'Assistant', userLabel: 'Visitor' }),
    });
