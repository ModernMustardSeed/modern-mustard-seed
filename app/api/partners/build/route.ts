 ...
 * GET powers the build station page: remaining slots + the partner's own
...
import { mintBuiltSuite, PARTNER_BUILD_QA_LIFT } from '@/lib/partner-build';
...
async function loadBuildPartner(supabase: NonNullable<ReturnType<typeof getSupabase>>, email: string): Promise<BuildAffiliate | null> {
  const { data } = await supabase
    ...
    .maybeSingle();
  const aff = data as BuildAffiliate | null;
...
export async function GET() {
  ...
  const aff = await loadBuildPartner(supabase, session.email);
  ...
  const [usedToday, usedWeek, mintsRes] = await Promise.all([
    usedCount(supabase, `partnerbuild:${aff.id}:day:${day}`),
    usedCount(supabase, `partnerbuild:${aff.id}:week:${isoWeekKey(new Date())}`),
    supabase
      .from('outbound_leads')
...
export async function POST(req: Request) {
  ...
  if (throttled(ip)) {
    return NextResponse.json({ error: 'slow_down', message: 'Easy there. The build takes a breath; try again in a bit.' }, { status: 429 });
  }
  const supabase = getSupabase();
  if (!supabase) return NextResponse.json({ error: 'not_configured' }, { status: 503 });
  ...
  if (!aff.can_forge) {
    return NextResponse.json(
      { error: 'forge_not_granted', message: 'The build is invite-only right now. Reply to any email from Sarah and ask for access.' },
      { status: 403 }
    );
  }
  ...
  if (!aff.forge_agreement_at) {
    if (body.agree !== true) {
      return NextResponse.json(
        { error: 'agreement_required', message: 'Accept the partner demo agreement to light the build.' },
        { status: 428 }
      );
    }
    ...
    if (agreeErr) {
      console.error('partner-build agreement stamp failed:', agreeErr.message);
      return NextResponse.json({ error: 'forge_failed' }, { status: 500 });
    }
  }

  const result = await mintBuiltSuite(supabase, 'partner', {
    affiliateId: aff.id,
    code: aff.code,
    email: aff.email,
