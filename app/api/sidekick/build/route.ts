 ...
 *   - 1 build per email, atomic via app_state pk claim (lib/sidekick-store)
...
import { buildCall, ringDemoCall, toE164, type SidekickProfile } from '@/lib/sidekick';
import {
  claimEmail,
  claimPhone,
  claimRing,
...
async function handleBuild(
  ...
  if (todayCount > GLOBAL_DAILY_CAP) {
    ...
    await notifySarah('VOICE AGENT BUILD: daily cap reached', [
      ...
      'Raise GLOBAL_DAILY_CAP in app/api/sidekick/build/route.ts if this is good traffic.',
    ]);
    return NextResponse.json(
      { error: 'forge_cooling', message: 'The build is cooling down after a busy day. Come back tomorrow, or call Mr. Mustard right now at ' + SIDEKICK.phoneLine + '.' },
      { status: 429 }
    );
  }
  ...
  if (!built.ok) {
    ...
    await releaseKey(supabase, 'email', email);
    console.error('sidekick build failed', built.error);
    return NextResponse.json({ error: 'forge_offline' }, { status: 503 });
  }
  ...
  return NextResponse.json({ runId, call: built.call, phoneLine: SIDEKICK.phoneLine });
}
