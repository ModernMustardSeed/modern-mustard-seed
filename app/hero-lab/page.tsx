...
import { HeroWall, HeroNight, HeroOffer } from '@/components/home/HeroDirections';
...
const DIRECTIONS = {
  a: { name: 'The Wall', hero: <HeroWall /> },
  b: { name: 'Night Screen', hero: <HeroNight /> },
  c: { name: 'The Offer', hero: <HeroOffer /> },
...
export default async function HeroLab({ searchParams }: { searchParams: Promise<{ d?: string }> }) {
  ...
  const key: Key = d && d in DIRECTIONS ? (d as Key) : 'a';
  return <>
    <StudioHome faq={LAB_FAQ} hero={DIRECTIONS[key].hero} showMine={key !== 'c'} />
    ...
    <nav aria-label="Hero directions" className="heroLab">
      {(Object.keys(DIRECTIONS) as Key[]).map(k => <Link key={k} href={'/hero-lab?d=' + k} scroll={false} aria-current={k === key ? 'page' : undefined}>{k === 'now' ? DIRECTIONS[k].name : k.toUpperCase() + ' · ' + DIRECTIONS[k].name}</Link>)}
    </nav>
  </>;
}
