...
/**
 * MODERN MUSTARD SEED scripts only. Cross + Covenant has its own studio and its
 * own script file at app/sarahcxc/scripts.ts; nothing from that house belongs
 * in this list, because /admin/youtube and its metadata brain both read
 * PROMPTER_SCRIPTS straight from here.
 *
 * The shared type leaves `kind` and `pillar` open so two studios can define
 * their own tabs. MMS narrows them back down, which keeps the old exhaustive
 * checking: a typo in a kind or a pillar is still a compile error here.
 */
