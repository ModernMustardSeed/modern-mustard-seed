export type PrompterScript = {
  id: string;
  kind: 'episode' | 'short' | 'sales' | 'ad' | 'cxc';
  episode: string;
  session: string;
  publish: string;
  pillar: 'BUILD' | 'SYSTEMS' | 'STEWARD' | 'STORY' | 'SALES' | 'ADS' | 'COVENANT';
  title: string;
  hook: string;
  directorNote: string;
  /**
...
/**
 * CROSS + COVENANT, not MMS. These run in the CXC voice (see
 * ~/.claude/skills/design/brands/cxc-voice.md), not Sarah's studio-teaching
 * voice: reverent, declarative, no pitch, no product, no link. They are the
 * video arm of the "eternal creatures" batch in content/cxc/eternal-creatures-batch.md
 * and they publish to CXC surfaces (IG Reels 9:16, X), never to the MMS channel.
 *
 * Doctrine posture, non-negotiable and load-bearing for this whole batch:
 * creation is included in redemption, the details are NOT spelled out, the hope
 * is real. No script here promises a specific animal is in heaven. CXC 6 and
 * CXC 7 say the gap out loud on purpose, because a comfort that has to be
 * invented does not survive contact with real grief.
 */
const ETERNAL_CREATURES: PrompterScript[] = [
  {
    id: 'cxc-kingdom-full-of-animals',
    kind: 'cxc',
    episode: 'CXC 1 · Reel 9:16',
    session: 'CXC · Eternal Creatures',
    publish: 'Use anytime',
    pillar: 'COVENANT',
...
/** Episodes, how-we-build batches, Sales Desk, Meta Ads, Shorts bank, Origin batch, By Request, CXC. */
export const PROMPTER_SCRIPTS: PrompterScript[] = [...GENERATED, ...BEHIND_THE_BUILD, ...MORE_SAUCE, ...SALES_DESK, ...META_ADS, ...TIGHT_CUTS, ...ORIGIN, ...AUDIENCE_REQUESTS, ...ETERNAL_CREATURES];
