import { getSupabase } from '@/lib/supabase';
import { getRun } from '@/lib/sidekick-store';
import { buildCall } from '@/lib/sidekick';
import { buildMetadata } from '@/lib/seo';
import DemoCallExperience from '@/components/sidekick/DemoCallExperience';
import Link from 'next/link';

export const metadata = buildMetadata({ title: 'Your Voice Agent Demo', noindex: true });
export const dynamic = 'force-dynamic';

/**
 * The shareable built demo: the cockpit builds a run for a lead, this page
 * resurrects it so the prospect can talk to "their" voice agent before the
 * meeting. Unlisted (unguessable run id, noindex); each call is capped
 * platform-side at 4 minutes.
 */
export default async function SidekickDemoPage({ params }: { params: Promise<{ runId: string }> }) {
  const { runId } = await params;
  const sb = getSupabase();
  const run = sb ? await getRun(sb, runId) : null;

  if (!sb || !run) {
    return (
      <div className="min-h-screen bg-[#FBF6EA] flex items-center justify-center px-6">
        <div className="max-w-md text-center bg-white border-2 border-[#161616] rounded-2xl shadow-[6px_6px_0_0_#161616] p-8">
          <h1 className="font-display text-3xl font-bold text-[#161616]">This demo has moved on</h1>
          <p className="font-body text-[#161616]/70 mt-3">
            We could not find that voice agent demo. Want one built for your business?
          </p>
          <Link
            href="/voice-agents/build"
            className="inline-block mt-5 bg-[#F5B700] text-[#161616] border-2 border-[#161616] rounded-xl px-6 py-3 font-sans font-bold uppercase tracking-[0.1em] text-sm shadow-[3px_3px_0_0_#161616]"
          >
            Build mine now
          </Link>
        </div>
      </div>
    );
  }

  const built = await buildCall(run, runId, 'web');

  // Send them back to THEIR order card (this pack's pricing), not the generic
  // Voice Agent page. Every built lead has a hub; legacy runs may not, and those
  // fall back to the old CTA.
  const { data: lead } = await sb
    .from('outbound_leads')
    .select('hub_demo_url')
    .eq('demo_run_id', runId)
    .limit(1)
    .maybeSingle();
  const orderUrl = lead?.hub_demo_url ? `${lead.hub_demo_url}#order` : null;

  return (
    <DemoCallExperience
      business={run.business}
      city={run.city}
      call={built.ok ? built.call : null}
      buildError={built.ok ? null : built.error || 'The demo line is warming up. Try again in a minute.'}
      orderUrl={orderUrl}
    />
  );
}
