...
export default function BookCall() {
  ...
  const [form, setForm] = useState({ name: '', email: '', business: '', focus: '', current: '', success: '', budget: '', timeline: '', startIso: '' });
  ...
  return (
    <form onSubmit={submit} className="max-w-2xl mx-auto space-y-8">
      ...
      <div className="glass-card p-7 md:p-8 space-y-4">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <Field label="Budget (optional)">
            <select value={form.budget} onChange={set('budget')} className={cls}>
      ...
      <div className="glass-card p-7 md:p-8">
        <span className="text-[10px] uppercase tracking-[0.3em] text-gold-light/80 font-mono font-bold block mb-1">Choose a time</span>
        <p className="text-white/45 font-body text-sm mb-5">30 minutes with Sarah. Wednesdays and Thursdays, Mountain Time.</p>
        {slotsLoading ? (
          <p className="text-white/40 font-body italic text-sm">Loading open times...</p>
        ) : slots.length === 0 ? (
          <p className="text-white/55 font-body text-sm">No open times in the next two weeks. Email <a href="mailto:sarah@modernmustardseed.com" className="text-gold-light underline">sarah@modernmustardseed.com</a> and we will find one.</p>
        ) : (
