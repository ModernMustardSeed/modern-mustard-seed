...
import copy from '@/data/ecommerce-campaign.json';
const BASE = '/ads/ecommerce';
...
export default function EcommerceCampaign() {
 return <div id="ecommerce-campaign" className="space-y-7 text-[#161616]">
  <section className="border-2 border-[#161616] bg-[#080C16] p-6 text-[#FBF6EA] shadow-[6px_6px_0_0_#F5B700] md:p-8">
   <p className="font-mono text-xs text-[#F5B700]">Ecommerce / Headless Shopify</p>
   <h2 className="mt-3 font-display text-4xl font-bold md:text-6xl">You make the product.<br /><span className="text-[#F5B700]">We build the store.</span></h2>
   <p className="mt-4 max-w-2xl leading-relaxed">The 38-second ecommerce commercial featuring Cross + Covenant, our own live store. Watch any cut here, download it, and copy the post below.</p>
   <a className={button+' mt-5'} href={BASE+'/ecommerce-campaign.zip'} download>Download the campaign</a>
  </section>
  <section className="grid gap-6 lg:grid-cols-2" aria-label="Ecommerce commercial videos">
   {[{id:'16x9',title:'Widescreen',width:1920,height:1080,note:'For LinkedIn, YouTube, and presentations.'},{id:'9x16',title:'Reels and Stories',width:1080,height:1920,note:'For Instagram, Facebook Reels, and Stories.'},{id:'1x1',title:'Square feed',width:1080,height:1080,note:'For Facebook, Instagram, and LinkedIn feeds.'}].map(cut=><article key={cut.id} className={card+(cut.id==='16x9'?' lg:col-span-2':'')}>
    ...
    <video aria-label={cut.title+' ecommerce commercial'} controls playsInline preload="none" width={cut.width} height={cut.height} poster={BASE+'/preview/poster-'+cut.id+'-800.webp'} src={BASE+'/build-your-store-'+cut.id+'.mp4'} className="mx-auto max-h-[650px] w-full bg-[#080C16]">
    ...
    <div className="flex flex-wrap gap-3"><a className={button} href={BASE+'/build-your-store-'+cut.id+'.mp4'} download>Download {cut.title}</a><a className={button} href={BASE+'/build-your-store-'+cut.id+'.mp4'} target="_blank" rel="noopener noreferrer">Open video</a><a className={button} href={BASE+'/poster-'+cut.id+'.jpg'} download>Download cover</a></div>
   </article>)}
  </section>
  <section className="grid gap-5 lg:grid-cols-2" aria-label="Ecommerce campaign copy">
  ...
  <section className={card}>
   ...
   <div className="mt-4 flex flex-wrap gap-3"><a className={button} href={BASE+'/captions.srt'} download>Download captions</a><a className={button} href={BASE+'/post-copy.txt'} download>Download post copy</a><a className={button} href="https://crossandcovenant.co" target="_blank" rel="noopener noreferrer">See the CXC store</a></div>
  </section>
 </div>;
}
