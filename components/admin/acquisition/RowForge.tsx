...
/**
 * FORGE AND SEND FROM THE ROW.
 *
 * Sarah, 2026-08-22: the forge belongs on the contact card, not only on the
 * Forge board. Scanning a list is where you actually notice that a business is
 ...
 * the same endpoint, so there is one definition of what a forge is.
 *
...
export default function RowForge({ id, business, email, suite, demoEmailedAt, onDone, push }: Props) {
  ...
  return (
    <span className="inline-flex flex-wrap items-center gap-1.5">
      ...
      {!built && !building && (
        <button
          onClick={() => void run('forge', 'forged. The website is on the anvil.')}
          disabled={busy !== ''}
