 ...
 *
 * The order is deliberate. RESEARCH first, so the operator sees what the Build
...
export default function FactoryBuild() {
  ...
  useEffect(() => {
    void (async () => {
      const [a, b] = await Promise.all([fetch('/api/admin/factories/build'), fetch('/api/admin/factories/tenants')]);
  ...
  const build = useCallback(async () => {
    if (!url.trim()) { setError('A website URL is where the Build starts.'); return; }
    setBusy('forge');
    setError(null);
    try {
      const res = await fetch('/api/admin/factories/build', {
        method: 'POST',
      ...
      if (!res.ok) throw new Error(json.error ?? 'The Build could not finish.');
      setResult(json);
      setTemplateKey(json.templateKey);
      if (!factoryName) setFactoryName(`${json.research.title?.split(/[|\-–]/)[0]?.trim() || 'New'} Factory`.slice(0, 60));
    } catch (err) {
      setError(err instanceof Error ? err.message : 'The Build could not finish.');
    } finally {
      setBusy(null);
    }
  ...
  return (
    <Shell>
      <AdminHeader active="factories" title="Build a Client Factory" />
      <Page>
        <PageTitle
          eyebrow="Client Factory Build"
          title={<>A website goes in. A <em className="font-display italic">reviewable blueprint</em> comes out.</>}
          sub="The Build reads their site, picks a template, and writes the configuration. You supply only the handful of facts no website carries."
        />
        ...
        <Card eyebrow="Step one" title="The business">
          <div className="space-y-5">
            <Field label="Their website" hint="The Build reads it and writes the blueprint from what it actually says. Nothing is invented.">
            ...
            <div className="grid md:grid-cols-2 gap-5">
              ...
              <Field label="Template" hint="Leave it on recommend and the Build picks from what the site says.">
            ...
            <div className="flex flex-wrap items-center gap-4">
              <Button tone="primary" onClick={() => void build()} disabled={busy !== null}>
                {busy === 'forge' ? 'Reading their site' : result ? 'Build again' : 'Research and build'}
              </Button>
        ...
        {result && (
          <>
            <Card eyebrow="What the Build read" title={result.research.title ?? result.research.url} right={<Badge tone={result.research.ok ? 'pass' : 'fail'}>{result.research.ok ? 'site read' : 'unreadable'}</Badge>}>
              {result.research.ok ? (
            ...
            <div className="grid gap-5 lg:grid-cols-2">
              ...
              <Card eyebrow="What it will take us" title="Implementation">
                {result.complexity ? (
                  ...
                  <Empty title="No blueprint to estimate from">Fix the issues below and build again.</Empty>
                )}
              </Card>
            </div>
