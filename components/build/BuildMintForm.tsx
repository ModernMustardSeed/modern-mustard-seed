...
/**
 * BUILD UNDER YOUR FLAG: the shared mint form. Two callers, one component:
 *   - /partners/hq/build  (endpoint /api/partners/build, variant 'partner')
 *   - /admin/hq/build     (endpoint /api/admin/hq/build,  variant 'admin')
 *
 ...
 * business name on it, and the status line burns "IN THE BUILD" while the
...
export default function BuildMintForm({ endpoint, variant }: { endpoint: string; variant: Variant }) {
  ...
  async function mint(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.status === 428) {
        ...
        setError(json.message || 'Accept the partner demo agreement to light the build.');
        return;
      }
      if (!res.ok) {
        setError(json.message || 'The build choked on that one. Try again in a minute.');
        return;
      }
      if (json.duplicate) {
      ...
      setError('The build is unreachable. Give it a minute.');
    } finally {
      setMinting(false);
    }
  }
  ...
  if (loading) {
    return <div className="bg-white border-2 border-[#161616] rounded-2xl shadow-[4px_4px_0_0_#161616] p-8 text-center font-mono text-xs uppercase tracking-[0.3em] text-[#161616]/50">Warming the build…</div>;
  }
  ...
  return (
    <div className="space-y-8">
      <style>{`
        @keyframes buildStamp{0%{transform:translateY(-6px) scale(1.01)}60%{transform:translateY(2px) scale(.995)}100%{transform:none}}
        ...
        @keyframes buildGlow{0%,100%{opacity:.55}50%{opacity:1}}
      `}</style>

      {/* Slots strip */}
      {(remainingToday != null || remainingWeek != null) && (
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-[9px] uppercase tracking-[0.25em] font-mono font-bold text-[#161616]/60">Build slots</span>
          {remainingToday != null && (
          ...
          >
            ...
            <div className="flex items-center justify-between mt-4">
              <span className="inline-flex items-center gap-2 text-[10px] uppercase tracking-[0.2em] font-mono font-bold text-[#161616]">
                <span className="w-2 h-2 rounded-full bg-[#E0301E] animate-[buildGlow_1.4s_ease-in-out_infinite]" />
      ...
      {/* The form */}
      {!ticket && (
        <form onSubmit={mint} className={`bg-white border-2 border-[#161616] rounded-2xl shadow-[5px_5px_0_0_#161616] p-6 md:p-8 ${minting ? 'animate-[buildStamp_.5s_ease-out_both]' : ''}`}>
