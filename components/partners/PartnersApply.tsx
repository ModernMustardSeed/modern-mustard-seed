...
export default function PartnersApply() {
  ...
  if (sent) {
    return (
      <div className="glass-card p-8 text-center max-w-lg mx-auto">
        ...
        <h3 className="font-display text-2xl font-semibold text-cream-50 mb-2">You're on the list</h3>
  ...
  return (
    <form onSubmit={submit} className="glass-card p-7 md:p-8 max-w-lg mx-auto space-y-4">
      ...
      {error && <p className="text-rust-light text-xs font-body">{error}</p>}
      <button type="submit" disabled={sending} className="w-full px-6 py-3.5 text-[11px] uppercase tracking-[0.2em] font-sans font-bold text-cream-50 bg-brass rounded-full campfire-glow hover:shadow-[0_0_30px_rgba(255,107,53,0.45)] transition-all disabled:opacity-50">
        {sending ? 'Sending...' : 'Apply to partner'}
      </button>
      <p className="text-white/30 font-body text-[11px] text-center">One structure for everyone. 50% on products, 10% of builds, free access to all of it.</p>
    </form>
  );
}

const inputCls = 'w-full bg-white/[0.03] border border-white/[0.08] rounded-lg px-4 py-2.5 text-sm text-white placeholder-white/25 focus:outline-none focus:border-mustard-500/40';
