...
/**
 * THE VOICE AGENT BUILD. The signature moment of /voice-agents/build.
 *
 * intake  -> a terminal card where you tell Mr. Mustard about your business
 * building -> the training montage: personalized log lines type out while the
 *            hero's mustard dots get pulled into the build (the persona is
...
import { SIDEKICK, sidekickVerticals, sidekickTiers, getVertical, buildScript, sidekickUsd } from '@/data/sidekick';
...
type BuiltCall = {
  ...
  /** Optional: runs built before 2026-07-21 (localStorage restores) predate it. */
...
type BuildResponse = { runId: string; call: BuiltCall; phoneLine: string };
...
export default function BuildExperience() {
  ...
  const buildApiDone = useRef(false);
  const montageDone = useRef(false);

  useEffect(() => () => { vapiRef.current?.stop(); }, []);

  // Restore a prior build so a returning visitor keeps their badge + context.
  useEffect(() => {
    try {
      const saved = localStorage.getItem('mms_sidekick_forge');
      if (saved) {
        const parsed = JSON.parse(saved) as { built: BuildResponse; form: typeof form };
        if (parsed?.built?.runId && parsed?.built?.call) {
          setBuilt(parsed.built);
          setForm(parsed.form);
          setStage('ready');
  ...
  // Keep the toggle in sync with whatever voice the built call carries.
  useEffect(() => {
    const vid = built?.call?.voice?.voiceId;
  ...
  const maybeReady = useCallback(() => {
    if (buildApiDone.current && montageDone.current) {
      setStage('ready');
  ...
  const build = async (e: React.FormEvent) => {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(f.email.trim())) {
      setError('A real email unlocks the build (your call summary lands there too).');
      return;
    }
    setSubmitting(true);
    setStage('forging');
    trackEvent('sidekick_forge_start', { vertical: f.vertical });
    buildApiDone.current = false;
    ...
    try {
      ...
      if (!res.ok) {
        const msg =
          ...
          (data?.error === 'rate_limited'
            ? 'Easy there. The build takes a breather between runs. Try again in a bit.'
            : 'The build sputtered. Give it another try in a minute.');
        setStage('intake');
        setSubmitting(false);
        setError(msg);
        return;
      ...
      if (!data?.runId || !data?.call?.firstMessage) {
        ...
        setStage('intake');
        setSubmitting(false);
        setError('The build sputtered. Give it another try in a minute.');
        return;
      }
      setBuilt(data as BuildResponse);
      try { localStorage.setItem('mms_sidekick_forge', JSON.stringify({ built: data, form: f })); } catch { /* fine */ }
      trackEvent('sidekick_forged', { vertical: f.vertical });
      buildApiDone.current = true;
      maybeReady();
    } catch {
      setStage('intake');
      setSubmitting(false);
      setError('The build lost its connection. Check your network and try again.');
    }
  };

  const startCall = async () => {
    if (!built || !PUBLIC_KEY || !ASSISTANT_ID) return;
    ...
    try {
      ...
      await vapi.start(ASSISTANT_ID, {
        firstMessage: built.call.firstMessage,
        model: built.call.model,
        transcriber: built.call.transcriber,
        startSpeakingPlan: built.call.startSpeakingPlan,
        stopSpeakingPlan: built.call.stopSpeakingPlan,
        backgroundSpeechDenoisingPlan: built.call.backgroundSpeechDenoisingPlan,
        silenceTimeoutSeconds: built.call.silenceTimeoutSeconds,
        maxDurationSeconds: built.call.maxDurationSeconds,
        metadata: built.call.metadata,
  ...
  const ring = async () => {
    if (!built || ringState === 'ringing') return;
    setRingMsg(null);
    ...
    try {
      const res = await fetch('/api/sidekick/build', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mode: 'phone', runId: built.runId, phone: cell, voice: gender }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setRingState('error');
  ...
  const buildEnabled = Boolean(PUBLIC_KEY && ASSISTANT_ID);
  ...
  return (
    <div className="relative">
      ...
      {/* ─── INTAKE ─── */}
      {stage === 'intake' && (
        <form onSubmit={build} className="rounded-2xl border-2 border-[#161616] bg-white p-6 md:p-8 shadow-[6px_6px_0_0_#161616]">
          <div className="flex items-center gap-3 mb-5 border-b-2 border-[#161616] pb-4">
            ...
            <div>
              <p className="font-mono text-[10px] uppercase tracking-[0.3em] text-[#C4160B] font-bold">Intake · The Build</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label htmlFor="sk-email" className={LABEL}>Your email (unlocks the build)</label>
          ...
          <button
            type="submit"
            disabled={submitting || !buildEnabled}
          ...
          >
            {buildEnabled ? 'Build my Voice Agent (free)' : 'The build is warming up. Check back shortly.'}
          </button>
          <p className="mt-3 text-center text-[11px] font-mono text-[#161616]/70">
            One free build per business. No card. He talks to you live when he graduates.
          </p>
        </form>
      )}

      {/* ─── BUILDING: the training montage ─── */}
      {stage === 'forging' && (
        <div className="rounded-2xl border-2 border-[#161616] bg-[#161616] p-6 md:p-8 shadow-[6px_6px_0_0_#F5B700] min-h-[380px]">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              ...
              <p className="font-display text-lg font-black text-[#FBF6EA] leading-tight">Mr. Mustard is building {possessive(form.business.trim())} front desk.</p>
            </div>
          </div>
          ...
          <div className="rounded-2xl border-2 border-[#161616] bg-white p-6 md:p-7 shadow-[6px_6px_0_0_#161616]">
            {ringState === 'rang' ? (
              <div className="text-center py-2">
                ...
                <p className="font-display text-2xl font-black text-[#161616]">Your phone is about to ring from {built.phoneLine}.</p>
            ...
            {ringState !== 'rang' && (
              <p className="mt-3 text-[11px] font-mono text-[#161616]/70">
                One demo call per number, ever. He calls from {built.phoneLine} and the whole thing is capped at four minutes.
              </p>
            )}
          </div>
        </div>
      ...
      {/* ─── KEEP HIM (always present below the build) ─── */}
      ...
      <div id="keep" className="pt-14 md:pt-20 md:relative md:left-1/2 md:-translate-x-1/2 md:w-[min(100vw-2.5rem,56rem)]">
        ...
        <div className="grid md:grid-cols-2 gap-6 max-w-3xl mx-auto">
          {sidekickTiers.map((tier) => (
            <PricingCard key={tier.slug} tier={tier} business={form.business.trim() || undefined} runId={built?.runId} />
          ))}
        </div>
