...
Most agentic projects fail at scope, not execution. The team picks the wrong workflow to automate, builds for a use case nobody asked for, or signs up for an open-ended engagement that drags into next quarter. The fix is a tighter front door.
...
Write down, in a single sentence, the job the agentic system is hired to do.

Bad: "Use automation to improve our customer experience."
...
By the end, you should have a flowchart on paper or in a Notion doc. Each step labeled with: actor, tool, input, output, time. The agent candidates are the steps where the actor is a human and the work is repetitive or cognitive.
...
- **Horizontal: agent fit.** How well do current models handle this? Score from 1 to 5.

The top-right quadrant is your roadmap. The top-left is "The models cannot do this yet, revisit in six months." The bottom-right is "An agent can do it but the impact is small, skip." The bottom-left is "neither relevant."
...
- **Human override** (when the agent hands back to a person).
...
Run this and you avoid the four most common agentic project failure modes:
...
4. **No human override.** Shipping an agentic system with no graceful fallback when it gets something wrong.

## Want this run on your project?

We run this exact conversation as the front door to every engagement. Two paid scoping calls inside our Audit + Roadmap product, or the first call of any larger build.

[Run the free audit](/audit) for a 60-second version, or [book a free call](/book) for the full thing.
