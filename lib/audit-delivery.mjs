...
export async function deliverFinishedAudits(sb, log = () => {}) {
  ...
  try {
    ...
    for (const job of jobs || []) {
      ...
      // A job some other table owns (an audit somebody requested from
      // /presence-audit, for one) is collected by its owner. Matching it to a
      // cold lead by hostname would file a requester's grade onto a prospect.
      if (job.source_table && job.source_table !== 'outbound_leads') continue;
