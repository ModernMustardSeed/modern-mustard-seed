...
/**
 * CLIENT FACTORY FORGE. The primary delivery system.
 *
 ...
 * Forge researches everything it can, then CONFIRMS what it found and ASKS for
...
export async function researchBusiness(rawUrl: string): Promise<BusinessResearch> {
  ...
  try {
    const res = await fetch(target.toString(), {
      ...
      redirect: 'follow',
      headers: { 'User-Agent': 'ModernMustardSeed-Forge/1.0 (+https://modernmustardseed.com)' },
    });
...
export type ForgeOutcome =
  | { ok: true; result: ValidateResult; draft: ForgeDraft; templateKey: string; missing: string[]; fitNotes: string | null }
 ...
 */
export async function forgeBlueprint(input: {
  ...
  try {
    draft = await llmJson<ForgeDraft>({
    ...
    return { ok: false, error: err instanceof Error ? err.message : 'The Forge could not generate a blueprint.' };
  }
...
/**
 * What the Forge still needs from a human, phrased as questions rather than
 * field names. Shown at the end of the wizard so the gap is obvious and small.
 */
export function outstandingQuestions(answers: ForgeAnswers, missing: string[]): string[] {
  const q: string[] = [];
  ...
  return [...q, ...missing.map((m) => `The Forge could not determine: ${m}`)];
}
