/**
 * THE FORGE WORKER'S HEARTBEAT.
 *
 * The demo-site worker runs on Sarah's own machine, not on Vercel. Both forge
...
import type { ForgeWorkerHealth, ForgeWorkerVitals } from '@/lib/outbound';

export async function readForgeWorkerVitals(db: SupabaseClient): Promise<ForgeWorkerVitals | null> {
  try {
