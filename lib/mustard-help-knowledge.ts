...
export function buildHelpKnowledge(): string {
  ...
  parts.push(
    `\n## Sales training (the full guide is at /admin/sales-training)\nWe sell three ways: calls, posts (online), and walk-ins. Newer reps are coached here.\n\nMindset: ${MINDSET.points
      ...
      .map((o) => `${o.q} -> ${o.a}`)
      .join(' ')} Voicemail: "${COLD_CALL.voicemail}" Cold call tips: ${COLD_CALL.tips.join(' ')} House rule (important): ${COLD_CALL.houseRule}\n\nAll prospecting and dialing happens on the Outbound floor at /admin/outbound (pinned in the top bar): the queue, scripts, audits, emails, demos, and outcomes, so the team sees activity and nobody double-calls. The old Lead Tracker at /admin/prospects was merged into Outbound and stays as a read-only archive. Reps should add and work businesses in Outbound. The Partner Hub at /admin/hq holds every teammate's partner code, ready-to-post swipe copy, the playbooks, the training, and free access to every program.\n\nThe signature walk-in voice agent play (let the live voice agent at /voice-agents demo itself and sell the deal): ${VOICE_DEMO_PLAY.steps
      .map((s) => `${s.label}: "${s.script}"`)
