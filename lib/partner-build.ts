/**
 * PARTNER-WIELDED BUILD: the shared mint. One code path serves both surfaces:
 *   - /api/partners/build  (approved partner with can_forge, origin 'partner')
 *   - /api/admin/hq/build  (internal team, origin 'rep')
 *
...
import type { OutboundLead, Niche, BuildOrigin } from '@/lib/outbound';
...
export const PARTNER_BUILD_GLOBAL_DAILY_CAP = 20;
export const PARTNER_BUILD_QA_LIFT = 3;
 ...
 * pitch of a partner mint is a suite built from the business's real evidence.
...
export async function mintBuiltSuite(
  ...
  if (!website) {
    return {
      ...
      message: 'A real website or Facebook page URL is required: the build works from their actual business, not from guesses.',
    };
  }
  if (email && !/.+@.+\..+/.test(email)) {
  ...
  const claims: Array<{ key: string; cap: number; error: string; message: string }> = [
    { key: `partnerbuild:${minterKey}:day:${day}`, cap: minter.dailyCap, error: 'daily_cap', message: `You have used all ${minter.dailyCap} of today's builds. The build resets at midnight UTC.` },
    { key: `partnerbuild:${minterKey}:week:${isoWeekKey(new Date())}`, cap: minter.weeklyCap, error: 'weekly_cap', message: `You have used all ${minter.weeklyCap} builds for this week. It resets Monday.` },
    { key: `partnermint:day:${day}`, cap: PARTNER_BUILD_GLOBAL_DAILY_CAP, error: 'capacity', message: 'The build hit today\'s global limit. Your slot is safe tomorrow morning.' },
  ];
  for (const c of claims) {
    const { data: claimed, error: capErr } = await supabase.rpc('claim_forge_slot', { p_key: c.key, p_cap: c.cap });
    if (capErr || claimed !== true) {
      if (capErr) console.error(`partner-build cap claim failed (${c.key}):`, capErr.message);
      return { ok: false, status: 429, error: c.error, message: c.message };
    }
  }

  const qaHeld = origin === 'partner' && minter.qaApproved < PARTNER_BUILD_QA_LIFT;
  const minterLine = origin === 'partner'
    ? `PARTNER BUILD: minted by partner ${minter.name}${minter.code ? ` (${minter.code})` : ''} for a business they personally know.`
    : `REP BUILD: prebuilt by ${minter.name} from the Partner Hub.`;
  ...
  const { data: leadRow, error: leadErr } = await supabase
    ...
    .insert({
      ...
      next_action: origin === 'partner'
        ...
        : 'Prebuilt by the team: dial with the suite live',
    })
    .select('*')
    .single();
  if (leadErr || !leadRow) {
    console.error('partner-build lead insert failed:', leadErr?.message);
    return { ok: false, status: 500, error: 'forge_failed', message: 'The build choked on that one. Try again in a minute.' };
  }
  let lead = leadRow as OutboundLead;

  // Build order proven by the Demo Station: voice (instant), website (queued
  ...
  if (hubMissing) console.error(`partner-build: hub mint failed for lead ${lead.id}; flagged in internal notify`);
  ...
  try {
    ...
    if (!synced.ok) console.error('partner-build pipeline sync failed:', synced.error);
  } catch (err) {
    console.error('partner-build pipeline sync threw', err);
  }
  ...
  if (process.env.RESEND_API_KEY) {
    ...
    try {
      await resend.emails.send({
        ...
        to: OWNER_NOTIFY_TO,
        subject: `${origin === 'partner' ? 'PARTNER' : 'REP'} BUILD: ${business} (${minter.name}${qaHeld ? ', QA HOLD' : ''})`,
        html: clientEmail({
          preheader: `${minter.name} minted a suite for ${business}.`,
          eyebrow: 'BUILD UNDER YOUR FLAG',
          greeting: `${minter.name} just minted one.`,
          body:
            `<p><strong>${business}</strong> (${contact}, ${phone}${city ? `, ${city}` : ''}) was built by ${minter.name} with origin <strong>${origin}</strong>.</p>` +
            (hubMissing
              ? `<p><strong>NEEDS A HAND:</strong> the demos built but the HUB MINT FAILED (lead ${lead.id}). Open the lead in the cockpit and rebuild the hub, then send the minter their hand-off.</p>`
              : `<p>Hub: <a href="${lead.hub_demo_url}">${lead.hub_demo_url}</a></p>`) +
            (qaHeld
              ? `<p><strong>QA HOLD:</strong> this partner has ${minter.qaApproved} of ${PARTNER_BUILD_QA_LIFT} approvals. Review it on the Partner Admin QA strip; the partner gets their hand-off email when you approve.</p>`
              : hubMissing
      ...
      console.error('partner-build internal notify failed', err);
    }

    if (!qaHeld && lead.hub_demo_url) {
      try {
        await sendBuildHandoffEmail(resend, { minterEmail: minter.email, minterName: minter.name, business, contact, hubUrl: lead.hub_demo_url });
      } catch (err) {
        console.error('partner-build hand-off email failed', err);
      }
    }
  }
 ...
 * clears review.
 */
export async function sendBuildHandoffEmail(
  ...
  await resend.emails.send({
    ...
    subject: `${first}, the suite you built for ${args.business} is live`,
    html: clientEmail({
      preheader: 'Their voice agent is ready now; the website lands at the same link.',
      eyebrow: 'YOUR BUILD, THEIR FLAG',
      greeting: `${first}, it is ready.`,
      body:
        `<p>The demo suite you minted for <strong>${args.business}</strong> is live at their private hub. The voice agent is ready right now; the custom website is designed from scratch, and it plus their walkthrough film land at the same link within 24 hours.</p>` +
