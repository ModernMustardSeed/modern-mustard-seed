...
- THE OUTLINE MOMENT (Sarah's taste, born on the Porsha Lee custom-paint hero; use it sometimes, where it fits). For trades with swagger (custom paint and auto body, barbershop, tattoo, street food, bold retail, gyms), the hero headline may MIX solid and outline words: most words filled, one or two rendered hollow with -webkit-text-stroke (transparent fill, stroke about 1.5 to 2.5px, and a solid-color @supports fallback), in a SECOND color from the palette so the line carries two colors. Big condensed display faces (Anton, Archivo Black, Bebas-class) carry it best. THE HOLLOW WORD IS NEVER FULLY TRANSPARENT OVER A PHOTOGRAPH. "color:transparent plus a hairline stroke" disappears the moment the plate behind it is dark or busy, which is exactly what happened to the word "UP" in Lined Up Barber Shop's own masthead on 2026-07-29. Build it as: a tinted fill of the accent at roughly 60 to 75% alpha, a stroke of 2.5 to 3px in a BRIGHTER tint of the same accent so the outline still reads as an outline against its own fill, a dark halo text-shadow, and a scrim placed exactly under the wordmark. It must survive the worst pixels behind it, not the average ones, and the contrast sweep now measures it (see VERIFY). Never in solemn registers, never more than one band per page, and the descender law above still applies to the stroked words.
...
ONE thing is legitimately unreadable and must not be "fixed": the door plates' description lines, which are meant to sit hidden at rest on a fine pointer and rise in on hover. Everything else at opacity 0 is a bug.
