 ...
 *
 * These two used to point at `${HOME}/wildmere` and `${HOME}/modern-mustard-forge`,
...
export const STYLE_ROTATION = `THE HOUSE STYLES, AND ROTATING BETWEEN THEM (2026-08-22; Sarah: "i want you to use all these diff styles as law and rotate between depending on whats best for that client")
SIX STYLES ARE ALREADY WORKED OUT, every one lifted off a build Sarah approved, with its real palette, its real type pairing and its real section skeleton. They are in the STYLES.md listed above. READ IT AND PICK ONE. Do not invent a seventh, and never blend two, because a hybrid reads as neither.
  1. STEEL AND EMBER (Wild Horse Construction) bg #F5F3EE, accent #DD4A17, Ultra + Archivo + Caveat. Trades that pour, weld, frame, finish.
  2. NIGHT NEON (Huck Yeah) bg #101614, accent #ff5a1f, Anton + Archivo, canvas ambience. Rentals, tours, powersports, anything sold on fun.
  3. BARBER RED (Columbia Falls Barbershop) bg #F4EDE1, accent #B0272C, Anton + Oswald + Source Serif 4. Shopfront trades with a price list.
  4. HIGHWAY AMBER (Hungry Horse Motel) bg #0B1018, accent #FF9A3D, Anton + Instrument Serif + Inter. Motels, lodges, diners, anywhere along a road.
  5. FIELD NOTE (Sands Surveying) bg #F4F1E8, accent #A84718, Archivo + IBM Plex Mono + Source Serif 4. Trades whose work produces figures.
  6. WILD REVERENT (Wild Hope) bg #0B1016, accent #E08A3C, Fraunces + Inter. Sold on feeling rather than price.
CHOOSE BY WHAT IS TRUE ABOUT THEM, not by what you like: pours and builds to 1, sold on fun to 2, shopfront price list to 3, along a road to 4, produces figures to 5, sold on feeling to 6. Never put the same style on two businesses in the same trade in the same town.
WHATEVER STYLE YOU PICK, THESE ARE NOT OPTIONAL, because they appear in every approved build and in none of the rejected ones: a MARQUEE, a live COUNTER, an accordion FAQ, one DRAG OR SLIDER moment, THREE type families, and a PROOF SECTION built on their real rating. The rejected builds averaged one font, one photograph and no proof section at all; one shipped with no webfont whatsoever.
...
Two defects shipped on the same day, on two builds, from two different engines. Both are layout failures, not taste, and both are banned outright.
...
VERIFY BOTH BEFORE YOU FINISH, by looking, not by intending. Load the page and read the top 700px at 1440 and at 390. Every word must be legible against what is actually behind it, and no two pieces of text may occupy the same pixels. A build where the business's own name is unreadable in the first screen is a FAILED build, no matter how good the rest is.`;
 ...
 *
 * ⚡ TWO FREE RENDERERS, BECAUSE THE FORGE RUNS ON MORE THAN ONE MACHINE
...
- ${AWARD_FORGE}/TEMPLATE.md
- ${AWARD_FORGE}/VARIANTS.md
- ${AWARD_FORGE}/REFERENCE.html (a real build that meets the bar; read its scrim, its signature moment and its closing band)
- ${AWARD_FORGE}/MOTION.md (the six signature-moment forms, and the traps in each)
- ${AWARD_FORGE}/STYLES.md (the six house styles, each lifted off an approved build; pick one and rotate)
 ...
 *
 * DELETED ONCE, ON 2026-08-22, by a merge that took master's whole copy of this file
 * and kept the worker's four references to this function. demo-site-worker.mjs names
 * it in a STATIC import, so its absence was not a degraded edit, it was a SyntaxError
 * at module link: the forge could not start at all, crash-looped 91 times, and built
 * nothing for two days while leads sat queued. scripts/check-worker-imports.mjs now
 * fails the build for exactly this, so it cannot be lost silently again.
 */
