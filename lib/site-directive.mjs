...
THE SHOWSTOPPER (mandatory; the thing the owner retells at dinner)
Every build ships ONE interactive set-piece themed to their EXACT trade, so specific it could not be moved onto another business's site. The owner must be able to PLAY with it, not just watch it. Vanilla JS + canvas/inline SVG only.
The flagship pattern, use it whenever the trade has a tool you can put in the visitor's hand: THE TRADE-TOOL CURSOR COMPANION. On fine-pointer screens the pointer carries a hand-drawn inline-SVG tool of their trade that ACTS, with a visible consequence:
- painter / auto body: a spray gun that lays real color mist (canvas particles) across a "paint this page" hero band
- pest control: a backpack-sprayer figure that mists; tiny drawn bugs scatter out of the fog
- plumber: a pipe wrench that drips; drops ripple where they land
- roofer: a hammer that taps; shingle chips kick off on click
- lawn / landscaping: a mower that cuts a lighter stripe through a grass-textured band
- bakery / restaurant: a whisk or pan that puffs flour dust or steam
- towing / auto repair: a tow hook trailing faint sparks along a road line
- electrician: a plug trailing a crackling thread of light
- salon / barber: scissors that snip; hair-line snippets drift down
- cleaning: a squeegee that wipes a smudged veil off the hero image on first pass
Invent the equivalent for any other trade: tool in hand, visible consequence, a grin in under a second. RULES: gate it behind matchMedia('(pointer: fine)'); the tool rides NEXT TO a small always-visible dot or the native cursor so links, buttons and inputs never get harder to hit; every effect layer is pointer-events: none; never touch scroll behavior. On touch screens ship the equivalent tap moment (a themed particle burst or wipe at the touch point) so a phone gets the grin too.
Where a playful cursor would cheapen the register (attorney, medspa, funeral, finance, clinic), go elegant instead at the SAME ambition: a scroll-driven hero scene that assembles, an interactive before/after reveal the visitor drags, or a luxurious living canvas (steam, silk, water light) that answers the pointer. Interactive, trade-specific, unforgettable; never skip it, never downgrade it to a hover state.

THE LIVING PAGE (always on, underneath the showstopper)
...
4. VERIFY like a skeptic. If a browser tool (Playwright) is available, open the file at 375px and 1440px widths, screenshot both, and LOOK: overflow, cramped spacing, unreadable contrast, broken layout, the bottom-right corner blocked. EXERCISE the showstopper: dispatch pointermove/click events across the hero and screenshot the consequence (mist laid, drops rippling, stripe cut); confirm a link and a button still click with the effect layer active. Then fix the three weakest things you see and re-check. If no browser is available, re-read the full file hunting the same failures.
