...
- PACING, WORK AND REST: the interactive sections (the menu, the ballpark machine, the second tool) are the page's loud beats, and loud beats never touch. Between any two of them there is always at least one calm editorial band (the proof wall, a full-bleed photo band, an oversized pull quote, a story or stat band) so the visitor gets a rest between tools. Two tools back to back reads like a software catalog instead of a business, and is a FAILED page no matter how good each tool is on its own.
...
SECTION BLUEPRINT (adapt names and order to the trade, keep the spine; the PACING law above governs adjacency)
...
2. Hero: THE PHOTOGRAPH (their own or generated) given the full editorial treatment, their name in the display face, a one-line promise written for their trade and city (specific, not "quality you can trust"), primary CTA "Call (their number)" + secondary CTA anchoring to the receptionist section, and THE FACT STRIP when the facts exist. The cursor companion rides over it. The tools do NOT live here.
3. Benefit band: three sharp, trade-specific reasons to choose them (not generic cards; vary the composition). Real story facts (years in business, family-owned, "best of") beat composed claims here.
4. Services: 4 to 6 real services for that trade, inline SVG icons, one or two sentences each written like someone who knows the work. Harvested service names win over invented ones.
5. THE MENU / STORE SECTION (include whenever the trade has anything orderable or browsable; food, retail, salons, medspas, and most services do): a beautifully art-directed menu, product grid, or service list, designed like the best restaurant menus and boutique shops online, so the owner SEES their catalog living on the site. Harvest their REAL menu/products/prices from the dossier when they exist; that is the strongest version. When you must compose items, write realistic trade-typical items but mark the section honestly with one elegant caption such as "Sample menu. Yours drops in when we build it for real." and prefix sample prices with "from". Never present invented items as their real offering without that caption. For order-style trades, buttons say "Call to order" and dial the real phone.
6. A REST BEAT whenever the menu shipped: one calm editorial moment between the menu and the machine (a full-bleed photo band, the oversized pull quote, the story band), so two dense sections never touch.
7. THE BALLPARK MACHINE: the estimator in its own titled band with a real section title in the owner's voice ("What would yours run?", "Get your ballpark"), a one-line prompt, and a nav anchor.
8. THE PROOF WALL: the social-proof act (see its law below), placed HERE, between the machine and the second tool. It is the page's natural rest beat, and evidence lands hardest right after the visitor has seen the value.
9. THE SECOND TOOL: its own titled band with its own nav anchor, always separated from the machine by the proof wall, never merged with it and never touching it.
10. THE RECEPTIONIST SECTION (the pitch inside the demo): a bold band explaining that this website ANSWERS ITS OWN PHONE, 24/7, and pointing at the gold button in the bottom-right corner: "Tap it. Pretend you are a customer." This section exists on every build; make it feel like a feature of THEIR business, not an ad for ours.
11. ONE MORE VALUE BAND where it earns its place (pick what serves this trade): an FAQ of 4 to 6 questions a real customer actually asks, answered with genuinely useful trade knowledge (not sales copy); a "what affects your price" transparency explainer; or a guarantee/process band. Value modules like this are what separate a demo that impresses from a demo that gets kept.
12. Service area: their city and surroundings, written warmly (a map is optional; a styled SVG region mark works). Real neighboring towns, never invented ones.
13. Contact: the real phone number HUGE and tappable, the designed hours with the open-now line when harvested, the address with its directions link when known. Any form must not pretend to submit; wire it to tel: or mailto:.
14. Final CTA band + footer with the demo credit line: "Demo built by Modern Mustard Seed · modernmustardseed.com".
...
A dedicated social-proof section designed editorially, never a row of three gray quote cards. It lives between the ballpark machine and the second tool, where it rests the page between its two loud beats and lands the evidence right after the visitor has seen the value.
