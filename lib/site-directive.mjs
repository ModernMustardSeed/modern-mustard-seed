...
4. VERIFY like a skeptic. If a browser tool (Playwright) is available, open the file at 375px and 1440px widths, screenshot both, and LOOK: overflow, cramped spacing, unreadable contrast, broken layout, the bottom-right corner blocked. Three checks are mandatory: (a) THE HERO IS A PHOTOGRAPH and it actually loaded, filling the hero with the type legible over it (a hero showing the toy, or a blank brand-colored box where the photo should be, is a failed build: fix it before anything else); (b) EXERCISE the cursor companion: dispatch pointermove across the hero and the page and screenshot the trace, then confirm a link and a button still click with the trace layer active; (c) EXERCISE the plaything in its own section: scroll to it, work its controls, screenshot the consequence (paint laid, bugs cleared, stripe cut), and confirm it sits below the fold and below the services. Then fix the three weakest things you see and re-check. If no browser is available, re-read the full file hunting the same failures.
