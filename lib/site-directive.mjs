...
IMAGERY FIRST (the template's rule): generate with fal.ai Seedream before building (key in ${falEnv}; pipeline notes: ${mediaNotes}). READ every generated asset before building around it. BiRefNet (fal-ai/birefnet/v2) for photo cutouts. Footage loops only if the final asset stays under 300KB; chroma-key footage per frame in JS, NEVER mix-blend-mode for footage. Their harvested real photos are proof and go in at gallery scale. If generation fails after a retry, reuse a matching real photograph from C:/Users/moder/mms-demo-sites/_library/index.json and regrade it. PHOTOS.md in this directory means REUSE those photographs, do not regenerate.
