...
- THE FIRST PERSON LAW (Sarah, 2026-09-03, on D&D Landscaping: "Let US know what WE are walking into", not "tell him what he is walking into"). THE SITE SPEAKS AS THE BUSINESS, NEVER ABOUT ITS OWNER FROM THE OUTSIDE. Every line the page says in its own voice is "we", "us", "our", or, for a one-person shop where it reads warmer, "I". Never "he", "him", "his", "she", "her", and never "{Owner} will call you", "{Owner} prices every job", "{Owner} and his crew". The owner opens the demo and reads about a stranger; the customer reads a brochure about someone who is not in the room. Third person is allowed in exactly two places: a customer's own words inside the proof wall, and the story chapter when it is written as a quote in the owner's voice. The owner's name still appears where it is personal rather than distant: the signature under the owner's letter, "Call Greg" on a button, the byline on a quote. lib/demo-quality.mjs reads the finished page for this and a page that speaks about its owner in the third person is REQUEUED with the offending lines handed back, and FAILED if it comes back that way twice.
...
2) "Pick a time on the calendar". An inline booking sheet inside the tool (inline expansion, never a modal): the next 6 to 8 working days as chips (skip days the trade does not run, label Today and Tomorrow), two or three time windows, name and phone inputs, and a request button disabled until day + window + a plausible phone exist. It ends in an HONEST requested state ("Requested: {day} · {window}. We text {phone} to confirm, usually the same day."), carrying the tool's result reference along, plus one small italic line: "Demo note: on the live site this request lands straight on your phone and calendar." Never claim a confirmed appointment, and never pretend a backend submitted.
