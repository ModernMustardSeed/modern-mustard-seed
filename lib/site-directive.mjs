...
TWO THINGS ARE LEGITIMATELY UNREADABLE AND MUST NOT BE "FIXED": type whose fill is transparent but which carries a real -webkit-text-stroke (that is THE OUTLINE MOMENT and it renders as an outline, so skip any element with a non-zero stroke width), and the door plates' description lines, which are meant to sit hidden at rest on a fine pointer and rise in on hover. Everything else at opacity 0 is a bug.
