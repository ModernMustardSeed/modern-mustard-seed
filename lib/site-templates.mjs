/**
 * THE SITE TEMPLATE REGISTRY: every visual system the Build is allowed to build.
 *
 ...
 *
 * Where a template comes from is recorded on it. Five were lifted off builds
 ...
 * to cover the trades the first six did not fit. A template Sarah rejects comes
 * OUT of this array and goes into RETIRED_TEMPLATES below, where it can be named
 * but never built.
 *
 * How a template reaches a build:
 *   - The cockpit and the Build board carry a picker: one template, or Random.
...
export const SITE_TEMPLATES = [
  {
    ...
    fits: ['construction', 'roofing', 'painting', 'garage_door', 'restoration', 'septic', 'home_services', 'plumbing', 'hvac', 'electrical', 'locksmith', 'landscaping', 'pest_control', 'towing', 'tree_service', 'auto_repair'],
...
- Copy: plain and proud, first person plural, short declaratives. Section titles are owned ("What a slab runs", "The work"), never abstract.`,
  },
  {
    ...
    fits: ['towing', 'auto_repair', 'moving', 'pool_spa', 'locksmith', 'garage_door', 'landscape_lighting'],
...
- Copy: editorial and calm. Distances, hours, what is nearby, what the morning looks like. Written for a traveller reading on a phone at 9pm.`,
  },
  {
    ...
    fits: ['professional', 'tree_service', 'septic', 'electrical', 'pest_control', 'hvac', 'plumbing', 'roofing', 'restoration', 'garage_door', 'landscaping', 'home_services', 'construction', 'attorney'],
...
- Copy: precise, unhurried, expert. Numbers where numbers exist. No hype.`,
  },
  {
    ...
    fits: ['restaurant', 'cafe_bakery', 'wedding', 'medspa', 'salon'],
...
DONE MEANS. The first viewport communicates place, product and primary action without scrolling; the gallery reads as a designed sequence, not six equal cards; the ritual and the recurring event are concrete and bookable; the offerings are complete enough to orient a visitor.`,
  },
  {
    ...
    fits: ['medspa', 'wedding', 'salon', 'real_estate', 'construction', 'dental', 'attorney', 'painting', 'pool_spa', 'landscape_lighting'],
...
- Copy: few words, set wide, third person for the house and second person for the guest. No exclamation marks, no superlatives. The specificity is the luxury: name the stone, the hour, the technique.`,
  },
  {
    ...
    fits: ['professional', 'attorney', 'electrical', 'hvac', 'real_estate', 'plumbing', 'roofing', 'restoration', 'septic', 'pest_control', 'dental', 'vet', 'cleaning'],
...
- Copy: declarative and short, nouns and numbers, headlines as statements of fact. Nothing is "passionate" and nothing is "bespoke".`,
  },
  {
    ...
    fits: ['cafe_bakery', 'restaurant', 'moving', 'cleaning', 'painting', 'salon'],
...
- Copy: punchy, warm, present tense, sounds like the chalkboard. Real names for everything on the lineup, real prices where the brief has them.`,
  },
  {
    ...
    fits: ['landscaping', 'tree_service', 'landscape_lighting', 'pool_spa', 'home_services', 'vet', 'medspa'],
...
- Copy: warm and practical. Seasons, timing, what to plant, what to cut, what a yard looks like a year from now. Written to a homeowner walking their lot.`,
  },
  {
    ...
    fits: ['dental', 'vet', 'medspa', 'professional', 'cleaning'],
...
/**
 * RETIRED TEMPLATES: a key that was in the registry, was built at least once,
 * and has been taken out for good.
 *
 * It stays named here rather than simply vanishing, for three reasons. Rows
 * already carrying the key have to resolve to something instead of throwing.
 * The admin has to be able to say what a stale row is wearing. And a name
 * written down as retired, with the reason attached, is much harder to
 * reinstate by accident than a name nobody can find.
 *
 * A retired key is NOT a template. siteTemplate() returns null for it, so it
 * can never be chosen in a picker, never be resolved by Random, and never
 * reach a builder. A stored row carrying one falls through to a fresh
 * weighted pick for that trade. scripts/check-retired-templates.mjs fails the
 * build if one of these keys ever appears in SITE_TEMPLATES again.
 */
export const RETIRED_TEMPLATES = [
  {
    ...
    name: 'Wild Reverent',
    retired: '2026-08-26',
    // What was actually rejected, so the gate can catch it coming back under a
    // different key: the deep-ink ground and the Fraunces display face together.
    palette: { ground: '#0B1016', accent: '#E08A3C' },
    type: { display: 'Fraunces' },
    why: 'Sarah, on the Whitefish Massage Therapy build: terrible, get rid of whatever it is so we do not make that again. The deep-ink church aesthetic (Fraunces at optical size 144 on #0B1016, the ember accent, THE TURN scroll moment) reads as a concept film about a feeling, not as a business a person is trying to book. It came from a church concept and it never stopped being one.',
  },
];

/**
 * A retired key, or null. For admin surfaces that have to name what a stale
 * row is wearing. Never a path back into a build.
 *
 * @returns {{key:string,name:string,retired:string,why:string} | null}
 */
export function retiredTemplate(key) {
  if (!key) return null;
  const k = String(key).trim().toLowerCase();
  return RETIRED_TEMPLATES.find((t) => t.key === k) ?? null;
}
 ...
 */
export const FIT_AVOID = 0;
export const FIT_NEUTRAL = 1;
export const FIT_DESIGNED = 3;

export function templateFit(t, trade) {
  if (!trade) return FIT_NEUTRAL;
  if (t.avoidFor.includes(trade)) return FIT_AVOID;
  if (t.fits.includes(trade)) return FIT_DESIGNED;
  return FIT_NEUTRAL;
}

/**
 * Resolve Random into one template key.
 *
 * FIT FIRST (2026-08-26). Random draws ONLY from the templates designed for the
 * trade. Before this, an unlisted template scored a neutral 1 and could win, so
 * a dental lead had a 70% chance of drawing a system built for somebody else
 * and a massage studio in Whitefish got a church. Every trade the Build serves
 * now has at least three templates designed for it, enforced by
 * scripts/check-template-fit.mjs, so drawing from that set costs no variety.
 *
 * The rungs, in order: designed for the trade and not excluded; designed for
 * the trade; merely allowed and not excluded; merely allowed; anything. The
 * exclusions (the lead's last template, what the same trade in the same town
 * wears) apply INSIDE a rung, never across one, so variety never buys a misfit.
 *
...
export function pickSiteTemplate({ trade = null, exclude = [], rand = Math.random } = {}) {
  ...
  const score = (t) => templateFit(t, trade);
  const designed = SITE_TEMPLATES.filter((t) => score(t) === FIT_DESIGNED);
  const allowed = SITE_TEMPLATES.filter((t) => score(t) > FIT_AVOID);

  // FIT BEATS VARIETY. A template designed for the trade wins over one that is
  // merely not forbidden, even when that means repeating a template the same
  // trade in the same town already wears. Two massage studios in Whitefish in
  // the same calm system, each in their own colours and photographs, is a much
  // smaller problem than one of them in a system built for somebody else.
  const ladder = [
    designed.filter((t) => !ex.has(t.key)),
    designed,
    allowed.filter((t) => !ex.has(t.key)),
    allowed,
    SITE_TEMPLATES,
  ];
  const pool = ladder.find((rung) => rung.length) ?? SITE_TEMPLATES;

  // Weights still apply on the neutral rungs; on the designed rung every
  // candidate scores the same, so the draw is even across them. The floor of 1
  // exists only so the last rung cannot divide by zero.
  const weight = (t) => Math.max(score(t), 1);
...
export function templateRosterLaw() {
  ...
  return `THE TEMPLATES, AND ROTATING BETWEEN THEM. ${SITE_TEMPLATES.length} visual systems are worked out in lib/site-templates.mjs, each with its palette, type pairing, skeleton and devices. When BRIEF.md carries a "${TEMPLATE_LINE} <key>" line, that template is already chosen and its law is at the top of this directive. When it does not, PICK ONE from this roster by what is true about the business, and never invent another or blend two. The rule Random follows applies to you: choose a template whose "For ..." line actually covers this business. A template that merely does not clash is the wrong answer, because that is how a massage studio ended up wearing a system built for a church:
${lines.join('\n')}
