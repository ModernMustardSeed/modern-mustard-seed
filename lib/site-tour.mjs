...
export function speakable(html) {
  return String(html || '')
    // Comments FIRST. A built page carries the build's own art-direction notes
 ...
 *
 * A built page legitimately carries scaffolding copy aimed at the OWNER while
...
function headingOf(chunk) {
  const m = chunk.match(/<h2[^>]*>([\s\S]*?)<\/h2>/i) || chunk.match(/<h3[^>]*>([\s\S]*?)<\/h3>/i);
  const h = speakable(m ? m[1] : '');
  return NEVER_SPEAK.test(h) ? '' : h;
}

/**
 * ⚠️ THIS USED TO BE A FIXED ANCHOR ALLOWLIST (switch/story/services/gallery/
 * menu/rooms/crew/reviews), and it matched almost nothing. Every design law in
 * lib/site-directive.mjs tells the model to name rooms and chapters in the
 * OWNER'S voice, not in a fixed vocabulary: SITE_LAW says "Rename them in the
 * owner's voice (The Case, The Table, The Bakehouse)", and tier 3 builds a
 * chapter id from the journey metaphor it invents per business. Black Knight
 * Construction's tier-3 build carried chapters "first-light", "the-list",
 * "roadside", "the-gate", "dusk", "home", "questions" — zero overlap with the
 * old allowlist, so only the welcome and close beats survived, one beat under
 * the floor, and the tour refused to publish. 2026-08-11.
 *
 * So the tour now reads the page's own STRUCTURE instead of guessing its
 * vocabulary: every <section id="..."> in document order is a candidate stop,
 * whatever the id happens to be. A bridge is picked by sniffing a handful of
 * trade-neutral keywords in the id and heading; anything unrecognized gets no
 * bridge at all; a bridge that says nothing about the business is always
 * literally true of it, and no bridge is even more true.
 */
const BRIDGE_RULES = [
  { test: /\b(?:menu|food|eat|dish|bakery|kitchen)\b/i, bridge: 'Here is what we serve.' },
  { test: /\b(?:gallery|portfolio|photos?|before-?after)\b/i, bridge: 'Have a look at the work.' },
  { test: /\b(?:service|list|catalog|offer|jobs?)\b/i, bridge: 'Here is what we actually do.' },
  { test: /\b(?:review|proof|testimonial|rating)s?\b/i, bridge: 'This is what people say afterwards.' },
  { test: /\b(?:crew|team|staff|who-?we-?are)\b/i, bridge: 'And the people you would be dealing with.' },
];
/** Sections reserved for the close beat, never spoken twice. */
const CLOSE_IDS = /book|contact|schedule|estimate|quote|arrival|home$/i;

function bridgeFor(id, heading) {
  const probe = `${id} ${heading}`;
  const hit = BRIDGE_RULES.find((r) => r.test.test(probe));
  return hit ? hit.bridge : '';
}

/** Every top-level <section id="..."> in the order it appears on the page. */
function sectionStops(body) {
  const opens = [...body.matchAll(/<section\b[^>]*\bid=["']([\w-]+)["'][^>]*>/gi)];
  return opens.map((m, i) => {
    const id = m[1];
    const start = m.index + m[0].length;
    const end = i + 1 < opens.length ? opens[i + 1].index : Math.min(body.length, start + 8000);
    return { id, start: m.index, end, chunk: body.slice(start, end) };
  });
}

/**
 * Build the whole spoken tour for one built site.
 *
...
export function buildTour(html, { business, phone = null } = {}) {
  ...
  // <nav> strip, because a built header is not always a <nav> element.
  const h1Pos = body.search(/<h1[^>]*>/i);
  const h1End = body.search(/<\/h1>/i);
  const stops = sectionStops(body);
  // Whichever <section> actually contains the <h1> IS the hero: the welcome
  // beat below already speaks its promise, so the stops loop must not speak
  // it again a breath later. Black Knight's "first-light" chapter is exactly
  // this section, and before this guard the hostess opened by saying its own
  // hero line twice in a row.
  const heroStop = h1Pos >= 0 ? stops.find((s) => s.start <= h1Pos && h1Pos < s.end) : null;
  const firstStop = heroStop ? heroStop.end : body.search(/<section\b[^>]*\bid=/i);
  ...
  for (const stop of stops) {
    if (heroStop && stop.id === heroStop.id) continue; // already spoken in the welcome
    if (CLOSE_IDS.test(stop.id)) continue; // that chapter speaks once, in the close beat
    const head = deNav(headingOf(stop.chunk));
    // Skip the heading in the quote so she does not say the same line twice.
    const said_ = deNav(quote(stop.chunk.replace(/<h[23][^>]*>[\s\S]*?<\/h[23]>/i, ' '), { max: 2 }));
    ...
    const bridge = bridgeFor(stop.id, head);
    beats.push({ id: stop.id, anchor: stop.id, text: [bridge, head, said_].filter(Boolean).join(' ').trim() });
  ...
  // ⚠️ NEVER NAME THE BUTTON'S COLOUR. The first draft said "press the gold
  // button", which is true on Glimmer and false on Kylers, whose accent is
  // rust. A bridge that describes the page has to be true of EVERY page, or it
  // is exactly the invented detail the rest of this file exists to prevent.
  // Position is the one property every built site shares.
  beats.push({
    id: 'close',
    anchor: book ? 'book' : 'top',
    text:
      `${bookLine || 'When you are ready, we would love to hear from you.'} ` +
      'You can ask right here, or press the button in the corner any time and talk to us now, even if it is late.',
  });
