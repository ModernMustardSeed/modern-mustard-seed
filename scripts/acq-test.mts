...
test('emails: every keypad key carries its digit and the numbers on the display', () => {
  ...
  const html = built!.html;
  const keys = [...html.matchAll(/<a href="([^"]*\/demos\?source=cold-email-calculator[^"]*)"[^>]*>([^<]{1,6})<\/a>/g)];
  ...
  // It lands on the free build, never on the page that asks for a phone number.
  assert.ok(!keys.some(([, u]) => u.includes('/mustard')), 'the keypad no longer lands on the callback page');
  assert.ok(keys.every(([, u]) => u.endsWith('#machine')), 'a tap lands on the machine, not the top of the page');
