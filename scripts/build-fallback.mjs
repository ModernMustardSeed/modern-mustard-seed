 ...
 *
 * Run: node scripts/build-fallback.mjs        (one job, then exit)
 *      node scripts/build-fallback.mjs --dry  (report what it WOULD do, spend nothing)
 */
...
// site-build-api runs the Claude Code CLI now, not the API, so there is no key
...
async function main() {
  ...
  const { data: slot, error: capErr } = await sb.rpc('claim_forge_slot', {
    p_key: `buildfallback:day:${new Date().toISOString().slice(0, 10)}`,
    p_cap: DAILY_CAP,
  });
  if (capErr || slot !== true) {
    log(`daily api build cap (${DAILY_CAP}) reached or claim failed${capErr ? ': ' + capErr.message : ''}. standing down.`);
    return;
  }
  ...
  log(`claimed. ${edit ? 'editing their site' : rebuild ? 'building their REAL site' : 'building the demo'} via the API...`);

  const t0 = Date.now();
  let result;
  try {
    if (edit) {
      const { editSiteWithApi } = await import('../lib/site-build-api.ts');
      ...
      const { buildSiteWithApi } = await import('../lib/site-build-api.ts');
      result = await buildSiteWithApi(
        String(claimed.brief ?? ''),
        String(claimed.business_name ?? 'the business'),
        { real: rebuild },
      );
  ...
  await sb.from('messages').insert({
    ...
    snippet: edit
      ? `Rebuilt from your prompt on the API because the workstation was ${workerAlive ? 'backed up' : 'offline'}. Live at ${siteUrl}.`
