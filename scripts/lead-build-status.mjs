#!/usr/bin/env node
/**
 * Was this lead's website built?
 *
 * Read-only. Matches a name against outbound_leads (business_name and
 * contact_name), then reports the real build state for each match: the
 ...
 *   node scripts/lead-build-status.mjs "Coy Smith"
 */
...
if (!needle) {
  console.error('Pass a name. Example: node scripts/lead-build-status.mjs "Coy Smith"');
  process.exit(1);
}
...
if (!leads.length) {
  ...
  console.log("Nothing was built for that name, because there is no such lead.");
  process.exit(0);
}
...
for (const l of leads) {
  ...
  if (!status) {
    console.log("  WEBSITE      NOT BUILT. No site_demo_status on the lead.");
  } else {
    const verdict =
      status === "ready" ? "BUILT, live" :
  ...
  if (l.site_demo_id) {
    ...
    if (!row) {
      console.log(`  build row    MISSING. Lead points at ${l.site_demo_id} but no outbound_demo_sites row exists.`);
    } else {
      console.log(`  build row    status=${row.status}  worker=${row.worker ?? "unclaimed"}`);
  ...
  // Catch orphans: rows built against this lead that the pointer lost track of.
  ...
  if (all.length > 1 || (all.length === 1 && all[0].id !== l.site_demo_id)) {
    console.log(`  all build runs for this lead (${all.length}):`);
    for (const r of all) {
