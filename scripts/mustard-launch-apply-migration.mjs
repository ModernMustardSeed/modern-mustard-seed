/**
 * MUSTARD LAUNCH activation:
 *   1. Applies supabase/migrations/040_mustard_launch.sql to the MMS CRM project
 *      via the Supabase Management API (same pattern as mustard-apply-migration).
 *   2. Verifies launch_runs and launch_progress respond through PostgREST.
 *
 ...
 *
 * Run: node scripts/mustard-launch-apply-migration.mjs
 */
...
const sql = readFileSync(path.join(process.cwd(), 'supabase', 'migrations', '040_mustard_launch.sql'), 'utf8');
console.log('Applying 040_mustard_launch.sql via Management API to', REF, '...');
...
for (const table of ['launch_runs', 'launch_progress']) {
...
console.log('MUSTARD LAUNCH tables verified.');
