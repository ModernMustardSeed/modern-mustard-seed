...
test('the graph describes one business and services reference it', () => {
  ...
  const service = serviceJsonLd({ path: '/agentic-websites', name: 'AI websites', description: SITE.description });
  assert.equal(service.provider['@id'], ORG_ID);
  assert.equal(service['@id'], `${SITE.url}/agentic-websites#service`);
...
test('canonical normalization strips campaigns and fragments and rejects external hosts', () => {
  assert.equal(canonicalUrl('/agentic-websites/?utm_source=chatgpt#prices'), `${SITE.url}/agentic-websites`);
  ...
  assert.equal(buildMetadata({ path: '/agentic-websites?utm_source=test' }).alternates?.canonical, `${SITE.url}/agentic-websites`);
});
...
test('sitemap contains public resources once and keeps operational routes out', () => {
  ...
  for (const path of ['/agentic-websites', '/resources', '/montana/kalispell', ...AI_RESOURCE_SLUGS.map((slug) => `/blog/${slug}`)]) assert.ok(urls.includes(`${SITE.url}${path}`), path);
...
test('AI referrers use hostname boundaries and distinguish UTMs from actual referrals', () => {
  ...
  assert.equal(attributionFrom(`${SITE.url}/agentic-websites?utm_source=chatgpt`, '').ai_source_evidence, 'utm');
  assert.equal(attributionFrom(`${SITE.url}/agentic-websites?utm_source=chatgpt`, 'https://perplexity.ai/').ai_source, 'perplexity');
...
test('campaign preservation respects existing parameters, fragments and protected destinations', () => {
  const context = attributionFrom(`${SITE.url}/agentic-websites?utm_source=chatgpt&utm_campaign=field-notes`, '');
