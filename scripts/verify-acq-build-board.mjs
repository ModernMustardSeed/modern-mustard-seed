/**
 * Run the build board's own logic against the REAL acquisition database and
 * print what it finds, so the numbers on the board are checked rather than
 * assumed.
 *
 *   node scripts/verify-acq-build-board.mjs
 *
 * Read-only. It never builds, never sends, and never writes a row.
 */
...
/* 1. everything already built */
const built = [];
for (let from = 0; ; from += 1000) {
  const { data, error } = await db
    .from('outbound_leads').select(LEAD_COLS).or(BUILT_FILTER)
    .order('updated_at', { ascending: false }).range(from, from + 999);
  if (error) throw new Error(`built: ${error.message}`);
  built.push(...data);
...
const counts = { building: 0, failed: 0, door: 0, warm: 0, opened: 0, consented: 0, called: 0, built: 0, sent: 0, cold: 0, closed: 0, all: 0 };
...
console.log('\n================ THE ACQUISITION BUILD BOARD, ON REAL DATA ================\n');
console.log('already built        :', built.length);
...
console.log('build worker          :', health?.value ? `${health.value.state}, last heard ${Math.round((Date.now() - new Date(health.value.at)) / 1000)}s ago` : 'never reported in');
console.log('\nBUCKETS');
...
console.log('Command centers built by hand, never shown to a prospect:', hiddenOs);
