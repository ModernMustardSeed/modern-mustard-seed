#!/usr/bin/env node
// "THE TALKING WEBSITE, FOR PLUMBERS" — set ten, the first VERTICAL cut.
//
// Same product as set nine, rewritten so every line names a plumber's actual
// day: hands under a sink, a ceiling dripping at eleven at night, four calls
// on the first hard freeze, the paperwork pile at nine PM. Built for reps to
// attach to outbound, so a prospect who never sees the rest of the library
// still gets something that looks made for them.
//
// Direction stays "Say Hello" (cream, mustard sunburst, halftone, the white
// hand-inked bubble on every card) because it is proven and it is the house
// look. What changes: **water blue #1E50C8 becomes the accent**, drips and a
// puddle replace the red dots on the dispatch board, and the scenes carry the
// trade instead of the mascot doing it.
//
...
// NO WRENCH. Tried twice (open-end spanner, then a pipe wrench with an
// asymmetric head) and both read as a tuning fork at card scale. The deeper
// problem was conceptual: the wrench belongs to the PLUMBER, and Mr. Mustard
// is the thing answering the phone while the plumber's hands are full. Giving
// him the tool muddles who is who. He keeps the handset; the trade lives in
// the copy, the drips, and the dispatch board.

// a falling drop of water, the accent motif of this whole set
const drop = (x, y, s, opacity = 1) =>
  `<path d="M${x} ${y} q${16 * s} ${26 * s} ${16 * s} ${40 * s} a${16 * s} ${16 * s} 0 0 1 ${
    -32 * s
  } 0 q0 ${-14 * s} ${16 * s} ${-40 * s} z" fill="${BLUE}" stroke="${INK}" stroke-width="${
    9 * s
...
function windowScene() {
  ...
  return `
    ...
    ${drop(px(320), up(660), G * 1.15)}
    ${drop(px(392), up(526), G * 0.85, 0.75)}
    ${drop(px(268), up(430), G * 0.62, 0.5)}
    <ellipse cx="${px(322)}" cy="${up(330)}" rx="${px(120)}" ry="${px(26)}" fill="${BLUE}"
             stroke="${INK}" stroke-width="${px(12)}"/>`;
}

// 03 — six lit rooms of the back office, drawn as a cutaway
const ROOMS = ['DISPATCH', 'ESTIMATES', 'INVOICES', 'PARTS', 'REVIEWS', 'FOLLOW UPS'];
function roomsScene() {
  ...
  ROOMS.forEach((label, i) => {
    ...
    out += `<g>
      ...
      ${drop(x + w - px(52), y + px(108), G * 0.72)}
    </g>`;
  });
  return out;
}
...
const LINES = [
  ['6:58 AM', 'Booked the water heater swap'],
  ['12:20 PM', 'Quoted the repipe and sent it'],
  ['10:41 PM', 'Took the burst pipe call'],
];
function sheetScene() {
  ...
  LINES.forEach(([t, txt], i) => {
    ...
    items += `<g>
      ...
      <text x="${x + px(96)}" y="${ly - px(6)}" font-family="JetBrains Mono, monospace" font-size="${px(19)}"
            font-weight="700" letter-spacing="${px(2)}" fill="${BLUE}">${t}</text>
...
const CARDS = [
  {
    ...
    head: 'IT ANSWERS<br>WHILE YOUR<br>HANDS ARE FULL.',
    sub: 'You are under a sink with a wrench in one hand. The phone rings anyway. Something picks up, in your company name, and books them in.',
    burstAt: [760, 400],
    tail: 'right',
  ...
  {
    id: '02-water-does-not-wait',
    eyebrow: '11:42 PM',
    head: 'WATER DOES<br>NOT WAIT FOR<br>OFFICE HOURS.',
    sub: 'Their ceiling started dripping at eleven at night. They called somebody. Whether that somebody was you came down to who picked up.',
    burstAt: [300, 430],
    tail: 'right',
    scene: windowScene,
  ...
  {
    id: '03-nine-pm-paperwork',
    eyebrow: 'The 9 PM pile',
    head: 'IT DOES THE<br>OFFICE WORK<br>TOO.',
    sub: 'Dispatch, estimates, invoices, parts, reviews, follow ups. The stack you work through after supper, already handled by the time you park the van.',
    burstAt: [540, 380],
    tail: 'left',
    scene: roomsScene,
  ...
  {
    id: '04-first-freeze',
    eyebrow: 'First hard freeze',
    head: 'FOUR CALLS<br>AT ONCE.<br>ONE OF YOU.',
    sub: 'Every one of them gets a real answer and a real time slot. Nobody sits on hold long enough to decide to try the next plumber on the list.',
    burstAt: [800, 420],
    tail: 'right',
    scene: crowdScene,
  ...
  {
    id: '05-already-dispatched',
    eyebrow: 'What it did today',
    head: 'BOOKED IT.<br>QUOTED IT.<br>WROTE IT DOWN.',
    sub: 'You never stopped working. The day showed up already dispatched.',
    burstAt: [820, 400],
    tail: 'right',
    scene: sheetScene,
  ...
  },
  {
    ...
    sub: 'Ask it what your customers ask you. Yours would answer in your company name, with your rates, your area, and your calendar.',
    burstAt: [280, 440],
    tail: 'left',
    scene: plateScene,
...
const card = (c) => {
  ...
  if (c.hand) {
    ...
    prop = `<svg class="prop" viewBox="0 0 ${P.vb[0]} ${P.vb[1]}"
      ...
      width:${Math.round(w)}px;height:${Math.round(h)}px;transform:rotate(-14deg)">${P.svg}</svg>`;
  }
